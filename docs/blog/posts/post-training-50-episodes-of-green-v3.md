...
Training was run in Google Colab using a jupyther notebook

Runtime was `free of charge`

  - RAM 16 GB
  - VRAM 16 GB on a T4 GPU

Free runtime alows `2` workers running

Sending a `batch_size=8` consumed 4.5 GB of VRAM

We saved `checkpoints` after each 1K of steps

We uploaded `checkpoints` to huggingface hub

We trained `20K` steps

Model pushed to huggingface: [pap_green_foam_in_boxpolicy-v3](https://huggingface.co/garagelab-duesseldorf/pap_green_foam_in_boxpolicy-v3)
